...
def symmetric_quantize(tensor, dtype=torch.int8):
    q_min = torch.iinfo(dtype).min
    q_max = torch.iinfo(dtype).max
    r_max = max(abs(tensor.min().item()), abs(tensor.max().item()))
...
def plot_quantization(original, quantized, dequantized, error, mae):
    ...
    plt.title(f'Symmetric Quantization and Dequantization\nMAE: {mae:.6f}', fontsize=14)
...
# Quantize
quantized_tensor, scale = symmetric_quantize(tensor)
...
Quantized tensor: tensor([  11,   53,   96,  -21, -128], dtype=torch.int8)
Scale: 0.009411765079872281
Dequantized Tensor: tensor([ 0.1035,  0.4988,  0.9035, -0.1976, -1.2047])

Mean Absolute Error (MAE): 0.483059
'''